...
const projects = ref([]), projectId = ref(1), docs = ref([]), systemStatus = ref({llm_configured:false,llm_model:null}), question = ref(''), messages = ref([]), loading = ref(false), syncing = ref(false), importing = ref(false), fileInput = ref(null), newProject = ref({name:'',repo_url:''}), repositoryUrl = ref('')
