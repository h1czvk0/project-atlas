...
<template>
  <main class="shell">
    ...
    <section class="grid">
      ...
      <section class="panel chat"><div class="panel-title"><div><h2>Atlas Assistant</h2><small>项目上下文 + RAG + Agent 工具</small></div><div class="session-actions"><select v-model="sessionId" title="历史会话" @change="openSession"><option :value="null">新对话</option><option v-for="session in sessions" :key="session.id" :value="session.id">{{ session.title }}</option></select><button title="开始新对话" @click="newConversation">＋</button></div></div><div class="messages"><div v-if="!messages.length" class="welcome"><div class="orb">✦</div><h3>快速理解这个项目</h3><p>选择一个真实场景开始：</p><div class="examples"><button @click="askExample('总结这个仓库的技术栈和主要模块')">仓库概览</button><button @click="askExample('项目如何启动？')">项目启动</button><button @click="askExample('新成员应该先阅读哪些文件？')">新人上手</button></div></div><TransitionGroup name="message"><div v-for="(m,i) in messages" :key="i" :class="['bubble',m.role]"><span>{{ m.role==='user'?'你':'Atlas' }}</span><div v-if="m.role==='assistant'" class="markdown-body" v-html="renderMarkdown(m.content)"></div><p v-else>{{ m.content }}</p><small v-if="m.tools?.length&&!m.meta">正在调用：{{ m.tools.join(' → ') }}</small><small v-if="m.meta">工具：{{ m.meta.used_tools.join(' → ') }} · {{ m.meta.confidence }}<br/>来源：{{ m.meta.sources.map(s=>s.document_name).join('、') || '无' }}</small></div></TransitionGroup><div v-if="loading" class="typing"><i></i><i></i><i></i><span>Atlas 正在检索项目上下文</span></div></div><form @submit.prevent="ask"><input v-model="question" placeholder="输入问题，例如：这个仓库的主要模块是什么？"/><button>发送 ↗</button></form></section>
    </section>
  </main>
</template>
