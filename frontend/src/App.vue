...
onMounted(async()=>{ await Promise.all([loadProjects(),loadSystemStatus()]); repositoryUrl.value=activeProject()?.repo_url||''; await loadDocs(); projectPoller=setInterval(async()=>{ if(isImporting()){ await loadProjects(); if(!isImporting()) await loadDocs() } },1200) })
