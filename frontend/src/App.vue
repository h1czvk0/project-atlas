...
const projects = ref([]), projectId = ref(1), docs = ref([]), question = ref(''), messages = ref([]), loading = ref(false), syncing = ref(false), fileInput = ref(null), newProject = ref({name:'',repo_url:''})
...
async function changeProject(){ messages.value=[]; await loadDocs() }
