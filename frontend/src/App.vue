...
async function openSession(){ if(!sessionId.value){ messages.value=[]; return } const rows=await fetch(`${API}/sessions/${sessionId.value}/messages`).then(r=>r.json()); messages.value=rows.map(row=>({role:row.role,content:row.content,meta:row.metadata_json||null,tools:row.metadata_json?.used_tools||[]})) }
  ...
  try{
    ...
    while(true){
      ...
      for(const block of blocks){
        ...
        if(event==='answer'){ reply.meta=data; sessionId.value=data.session_id||sessionId.value; reply.content=data.answer; loadSessions() }
...
onMounted(async()=>{ await Promise.all([loadProjects(),loadSystemStatus()]); syncProjectForm(); await Promise.all([loadDocs(),loadSessions()]); projectPoller=setInterval(async()=>{ if(isImporting()){ await loadProjects(); syncProjectForm(); if(!isImporting()) await loadDocs() } },1200) })
onBeforeUnmount(()=>clearInterval(projectPoller))
</script>
<template>
  <main class="shell">
    ...
    <section class="grid">
      ...
      <section class="panel chat"><div class="panel-title"><div><h2>Atlas Assistant</h2><small>项目上下文 + RAG + Agent 工具</small></div><div class="session-actions"><select v-model="sessionId" title="历史会话" @change="openSession"><option :value="null">新对话</option><option v-for="session in sessions" :key="session.id" :value="session.id">{{ session.title }}</option></select><button title="开始新对话" @click="newConversation">＋</button><button v-if="sessionId" class="delete-session" title="删除当前对话" @click="deleteConversation">×</button></div></div><div class="messages"><div v-if="!messages.length" class="welcome"><div class="orb">✦</div><h3>快速理解这个项目</h3><p>选择一个真实场景开始：</p><div class="examples"><button @click="askExample('总结这个仓库的技术栈和主要模块')">仓库概览</button><button @click="askExample('项目如何启动？')">项目启动</button><button @click="askExample('新成员应该先阅读哪些文件？')">新人上手</button></div></div><TransitionGroup name="message"><div v-for="(m,i) in messages" :key="i" :class="['bubble',m.role]"><span>{{ m.role==='user'?'你':'Atlas' }}</span><details v-if="m.role==='assistant'&&m.stages?.length" class="activity" :open="!m.meta"><summary>{{ m.meta?'查看执行过程':'正在执行' }}</summary><div v-for="(stage,index) in m.stages" :key="stage.key" :class="['activity-step',{current:!m.meta&&index===m.stages.length-1}]"><i>{{ !m.meta&&index===m.stages.length-1?'':'✓' }}</i><div><b>{{ stage.label }}</b><small>{{ stage.detail }}</small></div></div></details><div v-if="m.role==='assistant'" class="markdown-body" v-html="renderMarkdown(m.content)"></div><p v-else>{{ m.content }}</p><small v-if="m.meta">{{ m.meta.answer_mode==='model'?'模型回答':m.meta.answer_mode==='structured'?'工具结果':'本地回退' }} · 工具：{{ m.meta.used_tools.join(' → ') }} · {{ m.meta.confidence }}<br/>来源：{{ m.meta.sources.map(s=>s.document_name).join('、') || '无' }}</small></div></TransitionGroup><div v-if="loading&&!messages.at(-1)?.stages?.length" class="typing"><i></i><i></i><i></i><span>Atlas 正在准备</span></div></div><form @submit.prevent="ask"><input v-model="question" placeholder="输入问题，例如：这个仓库的主要模块是什么？"/><button>发送 ↗</button></form></section>
    </section>
  </main>
</template>
