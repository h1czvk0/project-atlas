...
async function openSession(){ if(!sessionId.value){ messages.value=[]; return } const rows=await fetch(`${API}/sessions/${sessionId.value}/messages`).then(r=>r.json()); messages.value=rows.map(row=>({role:row.role,content:row.content,meta:row.metadata_json||null,tools:row.metadata_json?.used_tools||[]})); scrollToLatest(true) }
